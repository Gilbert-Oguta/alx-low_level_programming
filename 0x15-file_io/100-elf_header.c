...
/**
 * check_elf - checks for a file that is an ELF file
 ...
 * @e_ident: pointer to arry that contains ELF magic numbers
 *
 * Return: magic numbers are separated by space difference
 */
void print_magic(unsigned char *e_ident)
{
	int token;
	...
	for (token = 0; token < EI_NIDENT; token++)
	{
		printf("%02x", e_ident[token]);

		if (token == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
...
/**
 * print_type - prints types of ELF header
 * @e_type: ELF type
 ...
 * @e_entry: address of EFL entry point
 * @e_ident: pointer of array that contains ELF class
 */
void print_entry(unsigned long int e_entry, unsigned char *e_ident)
{
...
/**
 * main - displays information that contains ELF header
 * at the begining of ELF file
 * @argv: array pointers to the arguments
 * @argc: number of arguments supplied to ptogram
 *
 * Return: Success (0)
 */
